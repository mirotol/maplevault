...
const EquipmentCard = ({ item }: EquipmentCardProps) => {
  const [detail, setDetail] = useState<Item | null>(item.metaInfo ? item : null);
    ...
    <div className="w-full min-h-[200px] bg-linear-to-b from-slate-800/95 via-slate-900/98 to-slate-950/98 border border-blue-500/30 rounded-lg shadow-xl p-5 text-white transition-all duration-300 hover:scale-[1.02] hover:-translate-y-1.5 hover:shadow-[0_20px_40px_-15px_rgba(251,191,36,0.3)] hover:border-amber-500/40 group flex flex-col relative overflow-hidden">
      ...
      <div className="flex-1 space-y-4 relative z-10">
        ...
        <div className="flex items-center justify-between border-b border-white/5 pb-2">
          <span className="text-[10px] text-gray-500 uppercase tracking-tighter font-bold">REQ LVL</span>
          <span className="text-sm font-bold text-yellow-500 group-hover:text-yellow-400 transition-colors">{requirements.level}</span>
        </div>
            ...
            <>
              {stats?.incPAD && stats.incPAD > 0 && (
                <div className="flex justify-between text-sm items-center">
                  <span className="text-gray-400 font-medium">Weapon Attack</span>
                  <span className="font-bold text-orange-400 group-hover:text-orange-300 transition-colors">+{stats.incPAD}</span>
                </div>
              )}
              {stats?.incMAD && stats.incMAD > 0 && (
                <div className="flex justify-between text-sm items-center">
                  <span className="text-gray-400 font-medium">Magic Attack</span>
                  <span className="font-bold text-blue-400 group-hover:text-blue-300 transition-colors">+{stats.incMAD}</span>
                </div>
              )}
            </>
          )}
      ...
      {!loading && stats?.tuc && stats.tuc > 0 ? (
        <div className="mt-4 pt-3 border-t border-white/10 relative z-10">
          <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest">
            ...
            <span className="text-amber-400/90 group-hover:text-amber-400 transition-colors">{stats.tuc}</span>
          </div>
        </div>
      ) : (
